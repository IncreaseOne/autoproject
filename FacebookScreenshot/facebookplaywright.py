...
class AutoScreenshot():
    ...
    async def screenshot(self, context):
        ...
        if await page.get_by_text("展开").count() > 0:
            ...
            for i in expands:
                try:
                    ...
                    pass
        while await page.locator("span", has_text=re.compile(".*已经到底啦.*|.*找不到任何结果.*|.*Join or Log Into Facebook.*")).is_visible(timeout=3000) is False:
        ...
        for i in need_to_screenshot:
            ...
            self.results.append({"image_name": f"{time.time()}.png", "image":screenshot_bytes, "link":link})
